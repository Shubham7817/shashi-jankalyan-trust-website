...
var decodeSurface = (image, signal) => new Promise((resolve, reject) => {
	const finish = (error) => {
		clearTimeout(timer);
...
var canvasFilterSupport = /* @__PURE__ */ new WeakMap();
...
var expand = (b, margin) => b.add(-margin, -margin, margin * 2, margin * 2);
var union = (a, b) => new Bounds(Math.min(a.left, b.left), Math.min(a.top, b.top), Math.max(a.left + a.width, b.left + b.width) - Math.min(a.left, b.left), Math.max(a.top + a.height, b.top + b.height) - Math.min(a.top, b.top));
...
var shadowSpace = (matrix, width, height) => {
...
var paintShadowSurface = async (ctx, paint, shadow, options, budget, space) => {
...
var paintBoxShadow = async (ctx, paint, shadow, options, budget) => {
